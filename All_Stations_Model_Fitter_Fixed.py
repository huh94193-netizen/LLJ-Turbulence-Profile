...
if SYSTEM_ZH_FONT:
    ...
    print(f"[System] 已启用中文字体: {SYSTEM_ZH_FONT}")
else:
    print("[System] 未检测到中文字体，将使用 Station ID (如 1443#) 代替中文名显示，以防止乱码。")
...
def model_ws_banta_norm(z_norm, alpha, beta):
    """
    Banta 风速模型 (归一化)
    U_norm = (z/Z_jet)^alpha * exp( beta * (1 - z/Z_jet) )
    """
...
def analyze_station(file_path):
    ...
    # --- 智能名称显示逻辑 ---
    raw_name = filename.split('-')[0] # 中文名 "双鸭山..."
    try:
        station_id = filename.split('-')[1] # ID "1443#"
    except:
        station_id = "Unknown"
        
    # 如果系统没有中文字体，强制使用 ID 作为显示名称
    if SYSTEM_ZH_FONT:
        display_name = raw_name
    else:
        display_name = f"Stn_{station_id}" # 使用英文 ID 防止豆腐块

    print(f"[{display_name}] 正在处理...", end=' ')
    ...
    if PLOT_FITS:
        ...
        axes[0].set_title(f'{display_name} WS (Banta)\nα={result["WS_Alpha"]:.2f}, β={result["WS_Beta"]:.2f}')
        ...
        axes[1].set_title(f'{display_name} TI (Inv-Banta)\nDip={result["TI_Dip"]:.3f}, Base={result["TI_Base"]:.2f}')
        ...
        axes[2].set_title(f'{display_name} WD (Linear)\nk={result["WD_Linear_k"]:.4f}')
        axes[2].set_xlabel('Delta WD [deg]')
        axes[2].grid(alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, 'plots', f'{display_name}_Fits.png'), dpi=100)
        plt.close()
...
def main():
    ...
    print(f"找到 {len(files)} 个数据文件，开始批量处理 (Banta Model)...")
